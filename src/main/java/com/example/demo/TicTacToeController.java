...
public class TicTacToeController implements Initializable {
    ...
    public TicTacToeController(Map map) throws IOException {
        if (sharedData.hasConnection()) {
            ...
            String playerToMove = retMap.get("PLAYERTOMOVE");
            String opponent = retMap.get("OPPONENT");

            if (sharedData.getPlayer().getName().equals(playerToMove)) {
                symbolPlayer = "X";
                symbolOpponent = "O";

                while (!response.contains("SVR GAME WIN") || !response.contains("SVR GAME LOSS") || !response.contains("SVR GAME DRAW")) {
                    ...
                    if (response.contains("SVR GAME YOURTURN")) {
                        ...
                        updateUI();
                    } else if (retMap.get("PLAYER").equals(opponent)) {
                        int move = Integer.parseInt(retMap.get("MOVE")); // get move opponent from server
                        // todo: update board with opponent move
                    }
                }
            } else {
                symbolPlayer = "O";
